...
static bool is_victus_s_thermal_profile(void)
{
    ...
    if (!board_name) {
        pr_info("HP WMI DEBUG: Board name is NULL\n");
        return false;
    }

    match_ret = match_string(victus_s_thermal_profile_boards,
                 ARRAY_SIZE(victus_s_thermal_profile_boards),
                 board_name);

    /* * Log the exact string inside single quotes to spot hidden spaces
     * and log the integer returned by match_string.
     */
